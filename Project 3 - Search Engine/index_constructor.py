...
def build_initial_index(title, headings, meta_texts, bold_texts, anchor_texts, remaining_text_str, body_content,
    ...
    meta_token = preprocess_text(' '.join(meta_texts))
    bold_anchor_token = preprocess_text(' '.join(bold_texts + anchor_texts))
    regular_token = preprocess_text(' '.join([remaining_text_str]))

    token_to_tag_value = {
        **{token: 3 for token in title_token},
        **{token: 1.5 for token in heading_token},
        **{token: 1.25 for token in meta_token},
        **{token: 1 for token in bold_anchor_token},
        **{token: 0 for token in regular_token}
    }
    ...
    for token in tokens:
        if token in token_to_tag_value:
            add_or_update_posting_list(token, initial_index, file_id, html_tag_value=token_to_tag_value[token])
    
    for first, second in zip(tokens, tokens[1:]):
        token = first + " " + second
        if first in token_to_tag_value:
            add_or_update_posting_list(token, initial_bigram_index, file_id, html_tag_value=token_to_tag_value[first])
...
def build_initial_index(title, headings, meta_texts, bold_texts, anchor_texts, remaining_text_str, body_content,
    ...
    """
    all_token_lists = {
        'title': preprocess_text(' '.join(title)),
        'heading': preprocess_text(' '.join(headings)),
        'meta': preprocess_text(' '.join(meta_texts)),
        'bold_anchor': preprocess_text(' '.join(bold_texts + anchor_texts)),
        'regular': preprocess_text(' '.join([remaining_text_str]))
    }
    ...
    for token_type, token_list in all_token_lists.items():
        for token in token_list:
            html_tag_value = html_tag_value_dict.get(token_type, 0)
